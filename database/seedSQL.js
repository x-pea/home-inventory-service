...
// Number of homes and rooms to create
const fakeHomesCount = 1000;
const fakeRoomsCount = 333;
...
const saveAllFakeReservations = firstHomeId => {
  ...
  for (let i = 0; i < fakeHomesCount; i += 1) {
    fakeReservations.push([
      faker.date.future(),
      firstHomeId + Math.round(Math.random() *
        (fakeHomesCount + fakeRoomsCount))
    ]);
  }
  return createSQL.connection.queryAsync('INSERT INTO reservations ' +
